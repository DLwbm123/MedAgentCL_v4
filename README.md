# MedAgentCL_v4 — latest experiment snapshot

Snapshot date: 2026-09-13. Focus: **Med-PRISM v2.4 RGPA**.
This repository contains the current server source, including uncommitted implementation changes,
plus selected aggregate evidence from the latest completed experiment. The upstream ms-swift
source and LICENSE are retained; this is a new snapshot history, not the original development history.

## Start here

- [Latest final report](reports/latest_rgpa/evaluation_lite_20260913_train3_gpu0/REPORT.md)
- [Machine-readable results](reports/latest_rgpa/evaluation_lite_20260913_train3_gpu0/summary.json)
- [Latest implementation](med_prism/rgpa/)
- [Training/evaluation entry](scripts/medicalskill_v2_4_rgpa/run_separated.sh)
- [Regression tests](tests/med_prism_rgpa/)
- [Implementation report](docs/med_prism_v2_4_rgpa/IMPLEMENTATION_REPORT.md)
- [Workflow documentation](docs/med_prism_v2_4_rgpa/SEPARATED_WORKFLOW.md)
- [Frozen RGPA contract](reports/latest_rgpa/rgpa_contract.json)
- [Run manifest](reports/latest_rgpa/run_manifest.json)
- [Publication scope and provenance](PUBLICATION.md)

## Latest measured result

MedicalSkill-CL-v1.2-lite-10k1k; three tasks; six lower-triangle evaluation cells,
1,000 examples per cell. Task 1/2 use accuracy; Task 3 uses all-concept micro-F1.
Workflow completion is PASS; the experimental decision is **NO-GO**.

| Metric (percent) | Baseline | RGPA | Delta (percentage points) |
|---|---:|---:|---:|
| R31 | 87.60000 | 87.60000 | +0.00000 |
| R32 | 43.10000 | 42.80000 | -0.30000 |
| R33 | 32.11955 | 31.93366 | -0.18590 |
| Avg3 | 54.27318 | 54.11122 | -0.16197 |
| BWT3 | -9.60000 | -10.25000 | -0.65000 |

The final result supersedes pending-result text in the earlier implementation report.
The workflow document describes an earlier GPU0-only plan; the final training-time artifact
records Task3 on GPUs 0 and 1, batch 1, accumulation 8, effective batch 16.
No T4/T5 or Candidate B results are claimed. Metrics are copied from the existing completed
run, not recomputed during publication. Raw predictions and checkpoints are not distributed.

Dependencies and earlier implementations remain because the latest experiment reuses them.
Older dataset-building artifacts and early smoke outputs are omitted.

---

## Upstream ms-swift README
