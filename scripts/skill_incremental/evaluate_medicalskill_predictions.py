#!/usr/bin/env python3
"""Evaluate MedicalSkill-CL predictions with fixed 0-100 metrics."""
...
def concepts(s): return {norm(re.sub(r"^(present|uncertain|negated)\s*:\s*","",x,flags=re.I)) for x in s.split(";") if norm(x)}
def lcs(a,b):
 old=[0]*(len(b)+1)
 for x in a:
  new=[0]
...
def main():
 ...
 for x in data:
  ...
  elif a.task==4:rs.append(rouge(pred,ref))
  elif a.task==5:ious.append(ground(pred,ref))
 ...
 if a.task in (1,2,6):
  ...
  if a.task==6:m["reasoning_rouge_l"]=100*sum(rs)/len(rs) if rs else 0
 ...
 elif a.task==4:m={"rouge_l_f1":100*sum(rs)/len(rs) if rs else 0}
 else:m={"accuracy_iou_0.5":100*sum(v>=.5 for v in ious)/len(ious) if ious else 0,"mean_iou":100*sum(ious)/len(ious) if ious else 0}
