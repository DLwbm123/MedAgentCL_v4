...
VERSION="medicalskill_concept_qwen3_v1"
SYSTEM="""Extract medical concepts from the caption only. Never infer from an image. Return JSON only with a concepts list. Each concept has canonical, type, status, laterality, mention. Allowed types: anatomy, finding, pathology, modality, device, procedure, attribute. status: present, uncertain, negated. laterality: left, right, bilateral, midline, unspecified. Keep only caption-supported concepts, preserve negation/uncertainty/laterality, exclude coordinates, boxes, percentages and generic prose, and deduplicate. Return at most 6 highest-value concepts as compact single-line JSON with no indentation."""
...
def parse(raw,cap):
 ...
 try: data=json.loads(m.group())
  ...
  data=repair_json(m.group(),return_objects=True)
 ...
 for x in items:
  ...
  if not can: continue
  if not mention:
   if can.casefold() in folded: mention=can
   else: continue
  elif mention.casefold() not in folded:
   if can.casefold() in folded: mention=can
   else: continue
  typ=typ if typ in TYPES else "finding"; stat=stat if stat in STATUS else "present"; lat=lat if lat in LAT else "unspecified"
...
def qa(path,out,seed):
 rng=random.Random(seed); sample=rng.sample(out,min(100,len(out))); mention=dup=neg=0
 for r in sample:
  ...
  for x in r["concepts"]:
   ...
   neg += x["status"]=="present" and bool(re.search(r"\b(no|without|absent)\b[^.;]{0,40}"+re.escape(x["mention"].casefold()),cap))
 report={"status":"PASS" if sample and mention==dup==neg==0 and not any(r["error"] for r in out) else "FAIL","reviewed_sample_count":len(sample),"manual_review_required":True,"caption_support_failures":mention,"duplicate_concepts":dup,"negation_errors":neg,"json_failures":sum(bool(r["error"]) for r in out),"empty_targets":sum(not r["target"] for r in out),"examples":sample}
...
def main():
 ...
 model=AutoModelForCausalLM.from_pretrained(a.model_path,local_files_only=True,trust_remote_code=True,torch_dtype=torch.bfloat16,device_map="auto").eval()
 ...
 for start in range(0,len(pending),a.batch_size):
  ...
  for src,cap,text in zip(batch,caps,raw):
   ...
   r={"id":rid(src),"source_caption":cap,"concepts":concepts,"target":target(concepts),"raw_response":text,"error":err,"model_id":MODEL_ID,"revision":REVISION,"model_path":a.model_path,"prompt_version":VERSION,"prompt_hash":phash(),"temperature":0,"do_sample":False}
