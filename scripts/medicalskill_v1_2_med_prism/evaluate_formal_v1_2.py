#!/usr/bin/env python3
"""Fresh-process reload and formal MedicalSkill-CL-v1.2 evaluation for one CL stage."""
...
ARTIFACT = Path(
    os.environ.get(
        "MED_PRISM_ARTIFACT_ROOT",
        "/remote-home/wangbomin/med_prism_medicalskill_v1_2_seed42",
    )
)
DATA_ROOT = Path(
    os.environ.get(
        "MED_PRISM_DATA_ROOT",
        "/remote-home/wangbomin/MedicalSkill-CL-v1.2",
    )
)
...
def evaluation_task_ids(stage: int) -> list[int]:
    """Return all tasks for zero-shot, otherwise only tasks learned so far."""
    return list(range(1, 6)) if stage == 0 else list(range(1, stage + 1))
...
def evaluate_rows(task_id: int, rows: list[dict[str, Any]], predictions: list[str]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if task_id in (1, 2):
        ...
        metric["primary_score"] = metric["all_concept_micro_f1"]
    elif task_id == 4:
        acc, means = [], []
        for row, raw in zip(rows, predictions):
...
def main() -> int:
    ...
    parser.add_argument(
        "--eval-limit",
        type=int,
        default=int(os.environ.get("MED_PRISM_EVAL_LIMIT", "0")),
        help="Per-task test limit; 0 evaluates the complete test split.",
    )
    ...
    evaluated_task_ids = evaluation_task_ids(args.stage)
    summaries = {}
    primary_cache = {}
    for mode in modes:
        for task_id in evaluated_task_ids:
            active_tasks = [] if args.stage == 0 else (list(range(1, args.stage + 1)) if mode == "primary_cumulative" else [task_id])
            if args.stage: set_active(model, active_tasks)
            dataset = DATA_ROOT / TASK_DIRS[task_id] / "test.jsonl"
            values = read_jsonl(dataset, args.eval_limit)
            if not values:
    ...
    final = {"status": status, "stage": args.stage, "evaluation_protocol": EVALUATION_PROTOCOL, "evaluated_task_ids": evaluated_task_ids, "matrix_scope": "base_zero_shot_all_tasks" if args.stage == 0 else "seen_tasks_only", "model_load_count": 1, "elapsed_seconds": time.time() - started, "data_root": str(DATA_ROOT), "eval_limit": args.eval_limit, "active_component_manifest": active, "reload_equivalence": probe, "cells": summaries}
