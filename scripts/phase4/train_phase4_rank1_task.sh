...
DATA=$ROOT/output/phase4_rank1/phase3_closure/data/smoke_train.jsonl

if [[ $# -ne 2 ]]; then
  echo "Usage: $0 TASK_ID OUTPUT_DIR" >&2
  exit 2
fi
TASK_ID=$1
OUT=$2
DEFAULT_CONFIG=$(printf "$ROOT/configs/phase4/task_%02d_rank1.json" "$TASK_ID")
CONFIG="${MED_PRISM_CONFIG_OVERRIDE:-$DEFAULT_CONFIG}"
...
"$PYTHON" -c 'import json,pathlib,sys; pathlib.Path(sys.argv[2]).write_text(json.dumps({"swift_sft_exit_code":int(sys.argv[1]),"checkpoint_exists":pathlib.Path(sys.argv[3]).is_dir(),"manifest_exists":pathlib.Path(sys.argv[4]).is_file()},indent=2)+"\n")' \
  "$swift_exit_code" "$OUT/exit_code.json" "$OUT/train/checkpoint-5" \
  "$OUT/train/checkpoint-5/rank1_manifest.json"
