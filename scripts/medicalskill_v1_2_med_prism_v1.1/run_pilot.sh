...
PILOT_ROOT=/remote-home/wangbomin/med_prism_v1.1_medicalskill_v1_2_pilot_seed42
CHECK_ONLY=0
...
bash scripts/medicalskill_v1_2_med_prism_versions/run_versioned_train.sh \
  --method-version 1.1 --pilot-mode --data-root "$PILOT_ROOT/data" \
  --output-root "$PILOT_ROOT/run" --gpus 0,1 --end-stage 3 \
  --shared-lr 1e-5 --private-lr 1e-4 --lambda-drift 0.01 --orth-lambda 0.1 "$@"
bash scripts/medicalskill_v1_2_med_prism_versions/run_pilot_evaluation.sh \
  --method-version 1.1 --data-root "$PILOT_ROOT/data" --output-root "$PILOT_ROOT/run" --gpus 0,1
