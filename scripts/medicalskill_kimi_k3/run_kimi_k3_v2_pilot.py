#!/usr/bin/env python3
"""Asynchronous, resumable Kimi K3 runner restricted to the frozen 500-ID v2 Pilot."""
...
def validate_content(raw: str) -> list[str]:
    ...
    for item in concepts:
        ...
        seen.add(clean)
    if not normalized:
        raise ValueError("concept_empty_after_exact_deduplication")
    return normalized
...
def active_success(cached: dict[str, Any] | None, source: dict[str, Any], prompt: dict[str, Any], endpoint: dict[str, str]) -> bool:
    ...
    return all((
        ...
        cached.get("normalization_hash") == prompt["normalization_hash"],
        cached.get("model_returned") == MODEL,
        cached.get("temperature") == 1.0,
        cached.get("top_p_sent") is False,
...
def build_result(
    ...
    return {
        ...
        "normalization_hash": prompt["normalization_hash"],
        "temperature": 1.0,
        "top_p_sent": False,
    ...
    for local_attempt in range(1, max_retries + 2):
        ...
        try:
            ...
            result = build_result(
                ...
                category, None, category if retryable else None,
            )
        ...
        if returned_model != MODEL:
            ...
            result = build_result(row, prompt, endpoint, credential_source, run_id, attempt_ordinal, returned_model, content, [], usage, latency, "contract_finish_reason_length", "FAIL", "finish_reason_length_at_fixed_1024", finish_reason, None)
        else:
            try:
        ...
        await writer.append(result)
        if result["status"] == "contract_finish_reason_length" and attempt_ordinal < 2:
            continue
        return result
    raise AssertionError("unreachable")


def load_contract(artifact: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    prompt = json.loads((artifact / "prompt_v2_manifest.json").read_text(encoding="utf-8"))
    schema = json.loads((artifact / "kimi_prompt_schema_v2.json").read_text(encoding="utf-8"))
    normalization = json.loads((artifact / "normalization_contract_v2.json").read_text(encoding="utf-8"))
    ...
    if prompt["temperature"] != 1.0 or prompt["max_completion_tokens"] != MAX_COMPLETION_TOKENS:
        raise RuntimeError("frozen_generation_contract_mismatch")
    if prompt.get("top_p_sent") is not False:
        raise RuntimeError("top_p_contract_mismatch")
    if sha_text(canonical_json(schema)) != prompt["schema_hash"]:
        raise RuntimeError("frozen_schema_hash_mismatch")
    if sha_text(canonical_json(normalization)) != prompt["normalization_hash"]:
        raise RuntimeError("frozen_normalization_hash_mismatch")
    prompt_contract = {key: prompt[key] for key in ("prompt_version", "system_prompt", "user_template", "model", "reasoning_effort", "temperature", "max_completion_tokens", "normalization_contract")}
    ...
    if selection.get("hard_unique_source_id_limit") != PILOT_LIMIT or len(selected_ids) != PILOT_LIMIT or len(set(selected_ids)) != PILOT_LIMIT:
        raise RuntimeError("pilot_hard_limit_contract_failed")
    return prompt, schema, normalization, selection
...
async def run(args: argparse.Namespace) -> int:
    ...
    prompt, schema, normalization, selection = load_contract(artifact)
    if args.config_audit:
        print(json.dumps({
            ...
            **endpoint, "model": MODEL, "reasoning_effort": REASONING_EFFORT,
            "temperature": 1.0, "top_p_sent": False, "max_completion_tokens": MAX_COMPLETION_TOKENS, "structured_output_mode": "json_schema_strict",
            "prompt_version": prompt["prompt_version"], "prompt_hash": prompt["prompt_hash"],
            "schema_version": prompt["schema_version"], "schema_hash": prompt["schema_hash"], "normalization_hash": prompt["normalization_hash"], "api_called": False,
        }))
        return 0

    selected_ids = list(selection["stable_record_ids"])
    frozen = {str(row["stable_record_id"]): row for row in read_jsonl(artifact / "concept_35k_candidate_manifest.jsonl")}
    if len(frozen) != 35_000 or any(row_id not in frozen for row_id in selected_ids):
    ...
    result_path = artifact / "v2_pilot_results.jsonl"
    ...
    async def worker(row_id: str) -> None:
        ...
        if args.canary and result["status"] != "success":
            stop.set()
        if result["status"] in {"contract_or_api_rejected", "contract_model_mismatch", "contract_invalid_output"}:
            stop.set()
    ...
    summary = {
        "status": "CANARY_PASS" if args.canary and run_ids[0] in active_cache else "STOPPED" if stop.is_set() else "RUN_COMPLETE",
    ...
    write_json(artifact / "v2_resume_state.json", summary)
...
def main() -> None:
    ...
    if args.concurrency < 1 or args.max_retries < 0 or args.max_api_calls < PILOT_LIMIT or args.max_api_calls > 650:
