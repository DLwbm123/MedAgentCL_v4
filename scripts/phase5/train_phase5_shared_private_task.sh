...
if [[ $# -ne 1 || ! "$1" =~ ^[12]$ ]]; then
  echo "Usage: $0 TASK_ID(1|2)" >&2
  exit 2
fi
TASK_ID=$1
CONFIG="$ROOT/configs/phase5/task_${TASK_ID}_shared_private.json"
if [[ "$TASK_ID" == 1 ]]; then
  DATA="$ROOT/artifacts/phase5_shared_private/data/task1_vqa_train.jsonl"
else
  DATA="$ROOT/artifacts/phase5_shared_private/data/task2_classification_train.jsonl"
fi
OUT="$ROOT/output/phase5_shared_private/runs/task_${TASK_ID}"
